<template>
  <table class="el-year-table" @click="handleYearTableClick" @mousemove="handleMouseMove">
    <tbody>
      <tr v-for="(row, fkey) in rows" :key="fkey">
        <template v-for="(cell, key) in row">
          <td v-if="!cell" :key="key" />
          <td v-else :key="key" class="available" :class="getCellStyle(cell)">
            <div>
